				...
				<form class="form-horizontal big-margin-top" method="{{$method}}" action="{{$url}}" enctype="multipart/form-data">
					{{ csrf_field() }}
				        ...
				        @if(Auth::user()->roll != env("ADMINISTRATOR"))
							...
							<label  class="col-md-4 control-label" id="birth_label" >@lang("project.birthday")</label>

                            <div class="col-md-6 " id="birth">
                                <input class="datepicker form-control valid" data-val="true" data-date-format="yyyy-mm-dd"  name="birthday" readonly="readonly" type="text" value="{{$user->user->birthday ? $user->user->birthday : ''}}">
							...
							<div class="form-group">
								<label class="col-md-4 control-label">@lang("project.about_me") </label>
								<div class="col-md-6">
									<textarea class="textarea" name="about">{{$user->phoneabout ? $user->about : ''}}</textarea>
								</div>
							</div>
							<div class="form-group">
							...
							@elseif(Auth::user()->roll == env("COMPANY"))
								<div class="form-group{{ $errors->has('id') ? ' has-error' : '' }}">
